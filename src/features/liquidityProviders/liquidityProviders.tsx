  ...
  async function updateBalances() {
    ...
    setData((oldData) => {
      for (let i = 0; i < oldData.length; i++) {
        let pool = oldData[i];
        pool.loanCurrency.balance = newBalances.get(pool.pool.info['0'])!;
        pool.collCurrency.balance = newBalances.get(pool.pool.info['1'])!;
      }
      return oldData;
    });
  }
      ...
      <>
        ...
        {!!liquidityPoolId && (
          <LiquidityPoolModal
            ...
            loanTokenPrice={
              priceMap[
                data.find((x) => x.pool.address == liquidityPoolId)!.pool.info[
                  '0'
                ]
              ]
            }
