...
export function Borrow() {
  ...
  let [simulatedLoans, setSimulatedLoans] =
    useState<PoolAndSimulationResult[]>();
      ...
      let pairsInfo = pools
        ...
        .filter(
          (thing, i, arr) =>
            arr.findIndex(
              (t) =>
                t.collAddress + t.loanAddress ===
                thing.collAddress + thing.loanAddress,
            ) === i,
        );
  ...
  const updateBalance = async () => {
    ...
    contract.read.balanceOf([address]).then((balance) => {
      ...
      setCollBalance(
        ((balance as bigint) / BigInt(10 ** collToken.decimals)).toString(),
      );
    });
    if (selectedLoan != undefined) {
      contract.read
        .allowance([address, selectedLoan?.pool.address])
        .then((allowance) => {
  ...
  const {
    ...
    onSuccess: (sentTxResult) => {
      ...
      sendNotification(
        ...
        'Borrow transaction sent successfully',
      );
    },
  });
  ...
  const { data: dataTxConfirmation, isLoading: isLoadingTxConfirmation } =
    useWaitForTransaction({ hash: currentTx });
  ...
  async function borrow() {
    ...
    if (collAllowance < rawValue) {
      writeApprove({
        args: [selectedLoan?.pool.address, rawValue],
        // @ts-ignore
      ...
      writeBorrow({
        args: [
          ...
          // set it to 99%
          (selectedLoan?.loan[0]! * BigInt(100 - borrowSettings.slippage)) /
            BigInt(100),
          // max repay limit: 1% slippage, so 101
          (selectedLoan?.loan[1]! * BigInt(100 + borrowSettings.slippage)) /
            BigInt(100),
  ...
  return (
    <Container>
      {pairs.length != 0 && priceMap != undefined && (
        <Grid>
          <SetupCol>
